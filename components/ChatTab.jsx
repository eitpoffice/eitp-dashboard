...
export default function ChatTab({ mode }) { 
  ...
  // --- BULLETPROOF READ TRACKER ---
  const [readTracker, setReadTracker] = useState({});

  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem('eitp_chat_tracker')) || {};
    setReadTracker(stored);
  }, []);
  ...
  // 2. Filter Active Chat & Fix "isMe" Identity Check
  const activeChats = useMemo(() => {
    if (!activeContact) return [];
    return messages.filter(m => {
      // CHECK BOTH ID AND NAME TO PREVENT MISMATCHES
      const isMeSender = String(m.sender_id) === String(currentIntern?.id) || m.sender_name === currentIntern?.name;
      const isMeRecipient = String(m.recipient_id) === String(currentIntern?.id) || m.recipient_name === currentIntern?.name;
  ...
  // 3. Auto-Mark as Read when Chat is Open
  useEffect(() => {
    if (activeContact && activeChats.length > 0) {
      const lastMsg = activeChats[activeChats.length - 1];
      setReadTracker(prev => {
        if (prev[activeContact.id] === lastMsg.id) return prev;
        const updated = { ...prev, [activeContact.id]: lastMsg.id };
        localStorage.setItem('eitp_chat_tracker', JSON.stringify(updated));
        return updated;
      });
    }
  }, [activeChats, activeContact]);

  // 4. Red Dot Notification Logic
  const hasUnread = (contactId) => {
    ...
    // If you sent the last message, hide the red dot.
    const didISendLast = String(lastMsg.sender_id) === String(currentIntern?.id) || lastMsg.sender_name === currentIntern?.name;
    if (didISendLast) return false;
    
    // If the last message ID doesn't match our tracker, show the red dot!
    return readTracker[contactId] !== lastMsg.id;
  };
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              {activeChats.map((msg, i) => {
                ...
                const isMe = String(msg.sender_id) === String(currentIntern?.id) || msg.sender_name === currentIntern?.name;
            ...
            <div className="p-3 bg-[#f0f2f5] flex flex-col">
              ...
              <form onSubmit={handleSend} className="flex items-center gap-3">
                ...
                <input 
                  ...
                  className="flex-1 bg-white border-none px-4 py-3 rounded-xl outline-none text-sm shadow-sm focus:ring-2 focus:ring-blue-100 transition-all" 
                  value={msgText} 
                  onChange={e => setMsgText(e.target.value)} 
                />
                <button type="submit" disabled={!msgText.trim() && !msgFile} className="p-3 bg-[#00a884] disabled:opacity-50 text-white rounded-full shadow-md transition-all shrink-0">
                  <Send size={18}/>
                </button>
              </form>
            </div>
